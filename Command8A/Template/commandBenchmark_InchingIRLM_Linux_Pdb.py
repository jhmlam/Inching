...
for pdbfn in pdbavail:
    ...
    #else:
    #    pass
    if os.path.exists("%s/PerformanceList_InchingIRLM_%s_%s_%s.pkl" %(Benchmarking_folder, pdbid, User_Platform, User_Device.replace(" ","") )):
        #if '1a1x' not in pdbid:
            continue
    print(pdbfn)
    st = time.time()
    ...
    if PART04_CalcualteEig:
        ...
        with open("%s/Eigval_InchingIRLM_%s_%s_%s.pkl" %(
        ...
        with open("%s/Eigvec_InchingIRLM_%s_%s_%s.pkl" %(
                    Benchmarking_folder, pdbid, User_Platform, 
                    User_Device.replace(" ","")),"wb") as fn:    
    ...
    if PART05_Performance:
        ...
        with open("%s/PerformanceList_InchingIRLM_%s_%s_%s.pkl" %(Benchmarking_folder, 
