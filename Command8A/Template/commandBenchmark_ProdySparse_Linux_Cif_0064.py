...
Benchmarking_folder = "../BenchmarkLinuxProdySparse0064_EPYC/"
...
print(len(pdbavail))
pdbavail = ["../DataRepo/PdbByAtomCount/1a1x.pdb"] + pdbavail
for pdbfn in pdbavail:
    ...
    if os.path.exists("%s/PerformanceList_ProdySparse_%s_%s_%s.pkl" %(Benchmarking_folder, pdbid, User_Platform, User_Device.replace(" ","") )):
    ...
    with open("%s/Eigval_ProdySparse_%s_%s_%s.pkl" %(Benchmarking_folder, pdbid, User_Platform, User_Device.replace(" ", "")),"wb") as fn:
        pickle.dump(np.array(eigval),fn, protocol=4)
    with open("%s/Eigvec_ProdySparse_%s_%s_%s.pkl" %(Benchmarking_folder, pdbid, User_Platform, User_Device.replace(" ", "") ),"wb") as fn:
    ...
    if PART05_Performance:
        ...
        performance = ["ProDy 2.4 (Sparse, ARPACK %s)" %(GPU), pdbfn, n_atoms, 
                        runtime, peak_mem, 
        ...
        with open("%s/PerformanceList_ProdySparse_%s_%s_%s.pkl" %(Benchmarking_folder, 
