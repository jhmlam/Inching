...
def OOC4_systemmatrix_Q_r_tol_JdCorrectedZ(system_matrix,
                Q,
                r,
                tol,
    ...
    #      As long as the matrix is kept positive definite it's okay to CG too.
    # NOTE Also See remark on section 4 in Voss '07 A new justification of the Jacobi–Davidson method for large eigenproblems

    solver = InchingLiteInteger.Burn.JacobiDavidsonHotellingDeflation.IterativeSolvers.gmres
...
def S_HeigvalRJDMHD_HeigvecRJDMHD(A , A_diag,
        
        k=16,
        tol=1e-10,
        maxiter=1000,
        User_CorrectionSolverMaxiter=200,
        ...
        User_FactoringToleranceOnCorrection = 1e-8,
        User_Q_HotellingDeflation = None,
        User_HotellingShift = 10.0,

        User_WantedIntervalLowerBound = None,
    ...
    if PART00_Initialization:
        ...
        #if User_WantedInterval is None:
        #    pass
        #else:
    ...
    for i_iter in range(maxiter):
        ...
        # ======================
        # NOTE NEW! Selecting modes
        # ======================
        # NOTE Selecting modes for rational filter
        WantedContained = True
        if User_WantedIntervalLowerBound is None:
            pass
        else:
            # NOTE Rotate 
            idx_wanted = cp.where(S > User_WantedIntervalLowerBound)[0]
            idx_unwanted = cp.where(S <= User_WantedIntervalLowerBound)[0]

            if idx_wanted.shape[0] == 0:
                WantedContained = False

            rotated_idx = cp.concatenate((idx_wanted,idx_unwanted))

            S = S[rotated_idx]
            W = W[:,rotated_idx]

            #if n_RitzConverged > 0:
            #    print(eigval_converged[n_RitzConverged-1], S)
        ...
        while True:
            ...
            if cur_tol > tol or ( n_RitzConverged != k-1 and len(S) <= 1):#  or (S[0] < User_WantedInterval[0]):
                break
            ...
            # NOTE if it is not inrange then
            
            # ==================================
            # Compile the converged and postprocessing
            # ===================================================
            # NOTE NEW! if it is not inrange then
            if WantedContained:
                eigval_converged[n_RitzConverged] = theta
                n_RitzConverged += 1
                # NOTE I.e. accepting Q_
                jaccept_Q += 1
            else:
                # NOTE We will still orthogonalize against it, just that we dont accept it!
                pass
        ...
        if User_WantedIntervalLowerBound is not None:
            if (theta > User_WantedIntervalLowerBound + 5e-2) & (n_RitzConverged == 0):
                # NOTE Until we reach near the target value, we need not spend so much time searching
                User_CorrectionSolverMaxiter_ = 20
            else:
                User_CorrectionSolverMaxiter_ = User_CorrectionSolverMaxiter
        ...
        z = OOC4_systemmatrix_Q_r_tol_JdCorrectedZ(system_matrix,
                        ...
                        maxiter=User_CorrectionSolverMaxiter_
                        )
        
        """
        # NOTE This is a horner expansion, but it does not improve :(
        for i_deg in range(0):

            r_ = ( cupy.ravel(z) - cupy.ravel(r))[:,cp.newaxis]
        ...
        z = InchingLiteInteger.Burn.Orthogonalization.T2.OOC2_qnext_Q_ICGSqnext(z,V[:,:jcurrent_V]) # NOTE Do not overdo this
            ...
            #print('haha')
            ## V[:,:jmin] = 
        #    z = InchingLiteInteger.Burn.Orthogonalization.T2.OOC2_qnext_Q_ICGSqnext(z,V[:,:W.shape[0]].dot(W[:,startidx_unwanted:]))
            #startidx_unwanted
            #pass
